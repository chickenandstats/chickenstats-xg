...
def aggregate_stints_df(pbp_df: pl.DataFrame, meta_lookup: pl.DataFrame) -> pl.DataFrame:
    ...
    stints = df.group_by(["season", "session", "game_id", "period", "stint_id"]).agg(
        ...
        h_xgf=(pl.col("context_xg") * (pl.col("event_team") == pl.col("home_team"))).sum(),
        a_xgf=(pl.col("context_xg") * (pl.col("event_team") == pl.col("away_team"))).sum(),
        h_cf=(
...
def main():
    ...
    meta_lookup = process_pbp_metadata(all_pbp_lazy)
    ...
    for season in track(seasons, description="Processing play-by-play data for RAPM regressions..."):
        year = str(season)[:4]
        year_file = rapm_pbp_dir / f"pbp_{year}.parquet"
        if not year_file.exists():
            continue

        for session in ["R", "P"]:
            season_pbp = pl.read_parquet(year_file).filter(pl.col("session") == session)
            if season_pbp.is_empty():
                continue

            stints = aggregate_stints_df(pbp_df=season_pbp, meta_lookup=meta_lookup)

            save_file = base_dir / "data" / "rapm" / "stints" / f"stints_{year}_{session.lower()}.parquet"
