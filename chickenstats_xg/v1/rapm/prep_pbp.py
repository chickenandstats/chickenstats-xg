...
def load_scored_xg(scored_dir: Path) -> pl.DataFrame:
    ...
    if not frames:
        raise FileNotFoundError(f"No scored base_xg parquets found in {scored_dir}. Run base_xg/score.py first.")
    ...
    return pl.concat(frames).group_by(["game_id", "event_idx"]).agg(pl.col("base_xg").sum()).collect()
...
def enrich_year(
    ...
    df = df.join(scored_xg, on=["game_id", "event_idx"], how="left").with_columns(pl.col("base_xg").fill_null(0.0))
...
def main() -> None:
    ...
    pbp_dir = base_dir.parent.parent / "raw_data" / "pbp"
