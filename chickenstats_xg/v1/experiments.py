...
def _importance_as_array(booster: xgb.Booster, feature_names: list[str], importance_type: str = "weight") -> np.ndarray:
...
def _params_base_xg(trial: optuna.Trial, spw: float, X_train: pd.DataFrame) -> dict[str, Any]:
...
def _params_context_xg(trial: optuna.Trial, spw: float, X_train: pd.DataFrame) -> dict[str, Any]:
    """Optuna param space for context_xg (depth-2 gbtree).

    max_depth=2 is the core structural constraint that prevents paths longer than 2
    features. Per-flag isolation interaction_constraints were removed: they blocked binary
    flags (is_rebound, is_scramble, strength_state) from competing against continuous
    features for tree allocation — all trees went to the continuous group and flags had
    zero importance despite meaningful signal (rebound goal rate 15% vs base_xg 11.3%).
    At depth=2, the only possible interaction per path is a 2-way combination, which is
    already safe without explicit constraints.
    colsample_bytree adds stochastic feature regularization: each tree uses a random
    50–100% of the 21 features, disrupting repeated fingerprint paths across trees.
    Previously omitted because interaction constraints required all features to be
    available in each constraint group — now that constraints are removed (Issue 21),
    ...
    return {
        ...
        # Ceiling raised from 200 → 500: EA top-50 had median lambda=153 and pressed against
        # the 200 ceiling; SH top-50 had p90=158 similarly. lambda directly shrinks leaf
        # update magnitudes (leaf_weight ∝ 1/(H+lambda)), so raising the ceiling lets the
        # model discover more aggressive L2 regularization to counter fingerprinting.
        # Floor raised from 1.0 → 10.0: passing models all had lambda ≥ 8.5 (PP minimum).
        # lambda < 10 produces bimodal output even with mds=1.
        "lambda": trial.suggest_float("lambda", 10.0, 500.0, log=True),
        "alpha": trial.suggest_float("alpha", 0.1, 10.0, log=True),
        "subsample": trial.suggest_float("subsample", 0.5, 1.0, step=0.05),
        "colsample_bytree": trial.suggest_float("colsample_bytree", 0.5, 1.0, step=0.05),
        # Ceiling lowered from 0.30 → 0.10 to prevent GOAL-side fingerprinting.
        # With N_ESTIMATORS_CONTEXT_XG=100 and EARLY_STOPPING_ROUNDS_CONTEXT_XG=20,
        # best_iter ≤ 80. At lr=0.10: max accumulated log-odds ≈ 0.10 × 80 × 0.385 = 3.1,
        # which caps context_xg at ~0.58 from a 5% base — preventing 99% fingerprint outputs.
...
def _objective(trial: optuna.Trial, data: ExperimentData) -> tuple[float, float]:
...
def _objective_body(trial: optuna.Trial, data: ExperimentData, current_run: Any) -> float:
    # pred_goal: logit(context_xg) as base_margin; context_xg col dropped (it IS the base_margin).
    ...
    if data.model == "pred_goal" and "context_xg" in data.X_train.columns:
        bm_train = logit(data.X_train["context_xg"].to_numpy())
        bm_test = logit(data.X_test["context_xg"].to_numpy())
        X_train = data.X_train.drop(columns=["context_xg"])
        X_test = data.X_test.drop(columns=["context_xg"])
    ...
    # context_xg: aucpr last → aucpr drives early stopping.
    #   base_margin=logit_base_xg means the model starts from a calibrated prior; logloss
    #   briefly INCREASES in early rounds as tree 0 overshoots, causing logloss-based
    ...
    fit_params = {**params, "eval_metric": eval_metric}
    ...
    pred_dist_df = pd.DataFrame(
        {
            ...
            "goals": np.percentile(y_probs[goals_mask], pct_labels).round(4)
            if goals_mask.sum() > 0
            else [float("nan")] * len(pct_labels),
            "non_goals": np.percentile(y_probs[shots_mask], pct_labels).round(4)
            if shots_mask.sum() > 0
            else [float("nan")] * len(pct_labels),
        }
    )
    ...
    if data.model == "context_xg":
        ...
        # GOAL-side fingerprint penalty — skipped for high base-rate states (base_rate >= 0.5).
        # For empty_against (~80% goal rate), goals legitimately exceed 0.85 — penalising this
        # would steer Optuna away from all well-calibrated models for that strength.
        goal_mask = y_np == 1
        if base_rate < 0.5 and goal_mask.sum() > 0:
            goal_fraction_high = float(np.mean(prob_platt[goal_mask] > 0.85))
            goal_fp_penalty = max(0.0, goal_fraction_high - _GOAL_FP_RAMP_START) * _GOAL_FP_WEIGHT
        else:
        ...
        mlflow.log_metrics(
            {
                ...
                "objective_goal_fp": goal_fraction_high,
                "objective_goal_fp_penalty": goal_fp_penalty,
                "objective_composite": composite,
            }
        )
        # Store goal_fp as a trial user_attr so select_top_trials() can filter without
        # querying MLflow (avoids a network round-trip per candidate during finalize).
        trial.set_user_attr("goal_fp", goal_fraction_high)
        # Return (composite, cal_ll) so NSGA-II directly penalises goal fingerprinting.
        # Previously returned (cal_prauc, cal_ll) — the goal_fp penalty was computed but
        # discarded, so NSGA-II steered toward bimodal models (high PR-AUC from perfect
        # goal/shot separation). With composite as the first objective, the Pareto front
        # excludes models with goal_fp > ~0.02 where the 3.0× ramp penalty overwhelms any
        # PR-AUC gain from fingerprinting. NOTE: existing studies must be nuked before
        # re-running — mixing old (cal_prauc) and new (composite) values[0] corrupts the
        # NSGA-II Pareto front since the two metrics are on different scales.
        return (composite, cal_ll)

    return (prauc, log_loss_val)
...
def tune_model(
    ...
    X_train, X_test, y_train, y_test, scale_pos_weight, pd_dataset = load_data(strength, study_name, model=model)
    ...
    # context_xg returns (composite, cal_ll); base_xg/pred_goal return (cal_prauc, cal_ll).
    first_metric = "composite" if model == "context_xg" else "cal_prauc"
    study.set_metric_names([first_metric, "cal_ll"])
    study.optimize(
            ...
            mlflow.log_metrics(
                {
                    ...
                    f"best_{first_metric}": max(t.values[0] for t in pareto),
                    "best_cal_ll": min(t.values[1] for t in pareto),
                }
            )
            # Log params from the Pareto trial with the best first objective as a representative.
            rep = max(pareto, key=lambda t: t.values[0])
            mlflow.log_params({"best_" + k: str(v) for k, v in rep.params.items()})
            mlflow.set_tag("best_trial_num", str(rep.number))
