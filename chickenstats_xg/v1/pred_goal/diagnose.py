...
Checks (per strength state):
  1. Distribution         — SHOT p90 / base_rate bimodal cliff check (fingerprinting detection)
  ...
  5. Lift over context_xg — pred_goal PR-AUC must exceed context_xg PR-AUC on hold-out
...
_NON_FEATURE_COLS = frozenset(["goal", "season", "context_xg"] + PASSTHROUGH_COLS)
...
def _predict(model, calibrator, df: pd.DataFrame, strength: str) -> np.ndarray:
    ...
    bm = logit(df["context_xg"].to_numpy()) if "context_xg" in df.columns else None
...
def check_distribution(scored_df: pd.DataFrame, strength: str) -> tuple[str, float]:
    ...
    Fingerprinting pushes non-goal SHOT predictions up, so SHOT p90 >> base_rate.
    High GOAL p90 is expected and correct — do not use GOAL/SHOT ratio.
    """
...
# ── Check 5: lift over context_xg ─────────────────────────────────────────────


def check_lift(strength: str, hold_df: pd.DataFrame, hold_preds: np.ndarray) -> str:
    """pred_goal PR-AUC must exceed context_xg PR-AUC on the same hold-out data."""
    if "context_xg" not in hold_df.columns:
        print(f"\n  Lift over context_xg: context_xg column missing — skip")
        return WARN

    base_prauc = float(average_precision_score(hold_df.goal, hold_df.context_xg))
    ...
    print(f"\n  Lift over context_xg (hold-out)")
    print(f"    context_xg PR-AUC: {base_prauc:.4f}")
    print(f"    pred_goal  PR-AUC: {pred_prauc:.4f}  (lift = {lift:+.4f})")
    print(f"  {status_icon(status)} Lift over context_xg [{status}]")
    return status
...
def check_feature_gain(strength: str) -> str:
    ...
    status = FAIL if top_ctx_share > CONTEXT_GAIN_FAIL else WARN if top_ctx_share > CONTEXT_GAIN_WARN else PASS
...
def check_hyperparameters(strength: str) -> None:
    ...
    if bi is not None and bi < 10:
        issues.append(
            f"INFO  best_iteration={bi} — thin adjustment layer on top of context_xg prior (expected when talent signal is weak)"
        )
    elif bi is not None and bi > 100:
        issues.append(
            f"INFO  best_iteration={bi} — substantive talent-layer learning detected; check lift vs context_xg"
        )
    ...
    if md is not None and md >= 5:
        issues.append(
            f"WARN  max_depth={int(md)} — deep trees with talent features can overfit to training-era RAPM patterns; check OOF gap"
        )
    if mcw is not None and mcw < 30:
        issues.append(
            f"WARN  min_child_weight={int(mcw)} below floor of 30 — leaf memorisation risk with sparse RAPM features"
        )
...
def check_holdout_metrics(
    ...
    p = hold_preds
    prior_p = hold_df.context_xg.to_numpy() if "context_xg" in hold_df.columns else None
...
def check_season_prauc(
    ...
    if "context_xg" in hold_df.columns:
        hold_scored["context_xg"] = hold_df["context_xg"].values
    ...
    has_base = "context_xg" in combined.columns
    print(f"\n  PR-AUC by season")
    ...
    for season in sorted(combined.season.unique()):
        ...
        base_str = f"{average_precision_score(sub.goal, sub.context_xg):>8.4f}" if has_base else f"{'—':>8}"
...
def run_strength(strength: str) -> tuple[dict[str, str], dict[str, float]]:
    ...
    if oof_path.exists():
        try:
            train_meta = pd.read_parquet(train_path, columns=["game_id", "event_idx", "goal", "season", "context_xg"])
        except Exception:
        ...
        keep_cols = [c for c in ["goal", "pred_goal", "season", "context_xg"] if c in merged.columns]
