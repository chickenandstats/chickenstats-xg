...
def classification_report(
    ...
    *,
    ...
    try:
        ...
        row_labels = [f"{cls} (n={sup:,})" for cls, sup in zip(classes, supports)] if support else list(classes)
    ...
    *,
    ...
    try:
        ...
        for i, (label, cls_name) in enumerate(zip(labels_int, classes)):
            ...
            fpr_dict[i] = fpr
            tpr_dict[i] = tpr
            ax.plot(fpr, tpr, color=colors[i], lw=1.5, label=f"{cls_name} (AUC={auc_val:.3f})")

        if n_classes > 1:
            # Macro average
            all_fpr = np.unique(np.concatenate(list(fpr_dict.values())))
            mean_tpr = np.mean([np.interp(all_fpr, fpr_dict[i], tpr_dict[i]) for i in range(n_classes)], axis=0)
            ...
            fpr_micro, tpr_micro, _ = sklearn.metrics.roc_curve(y_true_bin.ravel(), y_proba.ravel())
            micro_auc = sklearn.metrics.auc(fpr_micro, tpr_micro)
            ax.plot(
                fpr_micro,
                tpr_micro,
                color=colors[n_classes + 1],
                lw=1.0,
                linestyle=":",
    ...
    try:
        ...
        if iso_f1:
            ...
            for f1_score in [0.2, 0.4, 0.6, 0.8]:
                ...
                mask = prec >= 0
                ax.plot(recall_pts[mask], prec[mask], color="#333333", alpha=0.2, linestyle="--", linewidth=0.8)
                ax.annotate(
                    f"F1={f1_score:.1f}", xy=(recall_pts[mask][-1], prec[mask][-1]), fontsize=7, color="#555555"
                )
        ...
        for i, (label, cls_name) in enumerate(zip(labels_int, classes)):
            ...
            color = colors[i]
            ax.plot(rec, prec, color=color, lw=1.5, alpha=line_opacity, label=f"{cls_name} (AP={ap:.3f})")
            if fill_area:
    ...
    *,
    ...
    try:
        ...
        for j, pred_cls in enumerate(classes):
            vals = cm[:, j]
            bars = ax.bar(x, vals, bottom=bottom, label=f"Predicted: {pred_cls}", color=colors[j], width=0.5)
            for bar, val in zip(bars, vals):
                if val > 0:
                    ax.text(
                        ...
                        va="center",
                        fontsize=9,
                        color="white" if j == n_classes - 1 else YB_KEY,
                    )
            bottom += vals
    ...
    try:
        ...
        for i in range(n_classes):
            for j in range(n_classes):
                ...
                ax.text(j, i, label, ha="center", va="center", color=txt, fontsize=10, fontweight="bold")
    ...
    *,
    ...
    return {
        "classification_report": classification_report(
            ...
            colormap=colormap,
            **{k: v for k, v in shared.items() if k != "palette"},
        ),
        "roc_auc": roc_auc(
            y_true,
            y_proba,
